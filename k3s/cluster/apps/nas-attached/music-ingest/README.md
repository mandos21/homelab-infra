...
- FileBrowser Quantum is kept internal-only for now.

Path consolidation is no longer relevant because the upload UI is not publicly exposed.
...
4. Reconcile `music-ingest` directly.
